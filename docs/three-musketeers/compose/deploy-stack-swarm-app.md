...
到此关于 Docker Compose 就已介绍完了。其实在之前的服务中就已经介绍了 Compose 在服务中的使用，借助 Docker Compose 又说明了三剑客之 stack
的使用 [Docker 栈](../../get-started/docker-stack.md)。

笔者本打算写一个关于 `docker stack deploy swarm` 的栗子，不过在看到官方 Github 上的一个栗子后就打算直接在这里展示。该栗子介绍的很清晰，按照
步骤来很简单的就能部署起来。文档为英文，很容易理解，翻译后又觉得体现不出原意，因此不做翻译。下面就是该示例：
